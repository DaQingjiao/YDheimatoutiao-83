<template>
  <div class="article-comments">
    ...
    <!-- 当前评论 -->
    <van-cell title="当前评论">
      <van-image slot="icon" round width="30" height="30" :src="comment.aut_photo" />
      <span style="color: #466b9d;" slot="title">{{ comment.aut_name }}</span>
      <div slot="label">
        <p style="color: #363636;">{{ comment.content }}</p>
        <p>
          <span class="p-span">{{ comment.pubdate | relativeTime }}</span>
  ...
  methods: {
    ...
    async onAddComment () {
      ...
      const { data } = await addComment({
        ...
        art_id: this.$route.params.articleId// art_id // 文章id，对评论内容发表回复时，需要传递此参数，表明所属文章id。对文章进行评论，不要传此参数。
      })
